...
// load 5 random gifs on page load
  ...
  async getGifs(type) {
    ...
    if (type === 'query') {
      ...
      let response = await fetch(`https://api.giphy.com/v1/gifs/search?api_key=${process.env.REACT_APP_GIPHY_KEY}&q=${this.state.input}&limit=${this.state.limit}&offset=${offset}&rating=G&lang=en`)
    ...
    if (type === 'addToGifs') {
      ...
      let response = await fetch(`https://api.giphy.com/v1/gifs/search?api_key=${process.env.REACT_APP_GIPHY_KEY}&q=${this.state.lastQuery}&limit=${this.state.limit}&offset=${offset}&rating=G&lang=en`)
  ...
  render() {
    ...
    return (
      ...
      <div className="App-attribution">
        <p>Designed by <a href='http://www.jordanvidrine.com'>Jordan Vidrine</a> using ReactJS + the GIPHY API</p>
      </div>
      </div>
    );
  }
